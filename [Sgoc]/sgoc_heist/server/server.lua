...
AddEventHandler('fc-safebomba', function(odebrane)
    local xPlayer = ESX.GetPlayerFromId(source)
    local _source = source
    local item = xPlayer.getInventoryItem('bob_big_explosives')
...
RegisterServerEvent('fc-skryptalom')
AddEventHandler('fc-skryptalom', function(odebrane)
    local xPlayer = ESX.GetPlayerFromId(source)
    local _source = source

    xPlayer.addInventoryItem('bob_plans', 1)
end)
