...
exports.qtarget:AddCircleZone("sgocprad", vector3(896.87, -2290.12, 30.47), 0.55, {
    name="sgocprad",
    useZ=true,
    --debugPoly=true
	}, {
...
RegisterCommand('kameratest', function()
TriggerEvent('fc-bobcatsafeopen')
end)
...
AddEventHandler('fc-skrzyniaklucze', function()
    if key == false then
        ...
        elektryk = true
        key = true
        print('SKRZYNIA KLUCZE CONNECTION')
    else
...
AddEventHandler('fc-szafkadestroy', function()
    print('87153')
    ESX.ShowNotification("numer telefonu to 87153")
end)

RegisterNetEvent('fc-camera:hack')
AddEventHandler('fc-camera:hack', function()
    if kamery == true then
    ESX.ShowNotification('Zainicjowano włamanie do Komputera')
    Wait(3000)
